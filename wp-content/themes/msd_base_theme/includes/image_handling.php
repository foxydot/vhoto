 ...
 */
function msdbase_first_image($post, $h = false, $w = false) {
  $first_img = '';
  ob_start();
  ob_end_clean();
...
function msdbase_get_image($post, $default = false, $h = false, $w = false) {
	...
	if(empty($image)){ //Defines a default image
	    $image = msdbase_first_image($rp->post)?msdbase_first_image($rp->post):get_bloginfo( 'stylesheet_directory' ).$default;
	  } 
	return $image;
}