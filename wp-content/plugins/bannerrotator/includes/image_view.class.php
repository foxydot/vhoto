<?php
	class UniteImageViewBanner {
		...
		public static function getUrlThumb($urlBase, $filename,$width=null,$height=null,$exact=false,$effect=null,$effect_param=null) {			
			...
			if($exact == true) $url .= "&t=".self::TYPE_EXACT;			
			if(!empty($effect)) {
				$url .= "&e=".$effect;
				if(!empty($effect_param)) $url .= "&ea1=".$effect_param;
			}			
			return($url);
		}
		...
		private function getThumbFilename() {
			...
			if(!empty($this->type)) $thumbFilename .= "_" . $this->type;
			...
			//Add postfix
			if(!empty($postfix)) $thumbFilename .= "_".$postfix;			
			$thumbFilename .= ".".$ext;			
		...
		private function handleImageEffects(&$imgHandle) {
			if(empty($this->effect)) return(false);
		...
		private function resizeImageSaveNew($filepath,$filepathNew) {			
			...
			//If the image don't need to be resized, just copy it from source to destination
